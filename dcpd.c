...
DEFINE_FREE( libusb_devhandle, libusb_device_handle*, if (_T) do_usbdev_cleanup(_T));
...
static libusb_device_handle* find_and_open_device(libusb_context* ctx)
{
    ...
    if (count < 0) {
        pr_err(
            "Error: Failed to get device list: %s\n", libusb_error_name(count));
        return NULL;
    }
...
static int enable_dcp_mode(libusb_device_handle* handle)
{
    ...
    ret = libusb_bulk_transfer(handle, EP_OUT, (void*)dcp_enable_cmd,
        sizeof(dcp_enable_cmd), &transferred, USB_TIMEOUT_MS);
    if (ret < 0) {
        pr_err(
            "Error: Failed to enable DCP mode: %s\n", libusb_error_name(ret));
        return -1;
    }
...
int do_pcm_rx(libusb_device_handle* handle)
{
    ...
    while (running) {
        ...
        /* Process data */
        out_len = process_data(
            rx_buf, transferred, dx_buf, &volume, &volume_changed);
